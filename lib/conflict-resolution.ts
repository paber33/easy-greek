...
export class ConflictResolver {
      ...
      if (!cloudLogDates.has(log.date)) {
        ...
        if (cloudIndex >= 0) {
          ...
          merged[cloudIndex] = {
            ...
            accuracy: Math.round(
              ((cloudLog.correct + log.correct) / (cloudLog.totalReviewed + log.totalReviewed)) *
                100
            ),
          };
        }
   ...
   */
  private getNewestLogs(
    cloudLogs: SessionSummary[],
    localLogs: SessionSummary[]
  ): SessionSummary[] {
   ...
   */
  private getNewestConfig(
    cloudConfig: SRSConfig | null,
    localConfig: SRSConfig | null
  ): SRSConfig | null {
